...
bool DFS(Graph *graph, int s, int t, vector<int> &path);
bool DFS_Util(Graph *graph, int s, int t, vector<int> &visited, vector<int> &p);
...
bool DFS(Graph *graph, int s, int t, vector<int> &path) {
    ...
    bool ret = DFS_Util(graph, s, t, visited, p);
    if (ret) {
        int tmp = t;
        while (tmp != s) {
            path_stack.push(tmp);
...
bool DFS_Util(Graph *graph, int s, int t, vector<int> &visited, vector<int> &p) {
    ...
    for (auto it = adj.begin(); it != adj.end() && !ret; it++) {
        if (!visited[(*it).first->idx] && (*it).second > 0) {
            p[(*it).first->idx] = s;
            ret = DFS_Util(graph, (*it).first->idx, t, visited, p);
        }
    }
    return ret;
}
