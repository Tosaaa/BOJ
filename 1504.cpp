...
int main() {
    ...
    int v1, v2;
    cin >> v1 >> v2;
    int s1 = dijkstra(graph, 0, v1 - 1);
    // cout << s1 << endl;
    int s2 = dijkstra(graph, 0, v2 - 1);
    // cout << s2 << endl;
    int v1_v2 = dijkstra(graph, v1 - 1, v2 - 1);
    // cout << v1_v2 << endl;
    int e1 = dijkstra(graph, v1 - 1, N - 1);
    // cout << e1 << endl;
    int e2 = dijkstra(graph, v2 - 1, N - 1);
    // cout << e2 << endl;

    int ans;
    if (s1 == INF || s2 == INF || v1_v2 == INF || e1 == INF || e2 == INF) {
        ans = -1;
    } else {
        ans = s1 + v1_v2 + e2;
        ans = min(ans, s2 + v1_v2 + e1);
    }
