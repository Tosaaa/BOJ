...
ll movePipe(vvi &map, vector<vector<vector<ll>>> &memo, int x, int y, int state) {  // state: 0 horizontal, 1 vertical, 2 diagonal
    if (memo[x][y][state] != -1) {
...
int main() {
    ...
    vvi map(N, vi(N));
    vector<vector<vector<ll>>> memo(N, vector<vector<ll>>(N, vector<ll>(3, -1)));
    REP(i, 0, N) {
        REP(j, 0, N) {
            cin >> map[i][j];
        }
